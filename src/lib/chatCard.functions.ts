...
- When medium is "code", propose codeTemplate (confetti/fireworks = celebration, kinetic = elegance, hearts = love, starfield = contemplative, ribbons = whimsical, "ai" = surprise generative). Also propose codeMotion (2-6 words describing motion feel) and codePalette (3-5 hex, background first) whenever you have a specific vision. Do not set regenerateImage for code cards. The full message renders INSIDE the coded card as personal-note text under the headline, so whenever you change the message on a code card, the card will re-render automatically.
- For follow-up edits on a code card, ONLY set the fields that should change (codeMotion, codePalette, codeTemplate, or message). Keep other fields null unless the sender asked to change them too.
